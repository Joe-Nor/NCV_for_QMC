...
def compute_logp_for_dataset(model, target_parity, dataset, batch_size, device, nmin, nmax,
    ...
    Collects all samples first, precomputes tensors once, then runs batched inference.
    If the model uses an MLP ΔK head, candidate ΔKs are (re)computed via the Fortran
    wrapper that returns both prefix + candidates.
    """
...
def main():
    ...
    # Overall log p(x) (even vs odd, all data)
    ax = axes[1, 2]

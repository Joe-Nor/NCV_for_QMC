...
def plot_one_model(ax_scatter, ax_hist, logq, logf, nh, parity_label, ckpt, color, paper=False):
    ...
    For numerator model: q ∝ n_h * f, so reference is log(n_h * f).
    """
    # Filter out nh=0 samples (log(0) = -inf) and non-finite values
    mask = (nh > 0) & np.isfinite(logq) & np.isfinite(logf)
    n_total = len(logq)
    ...
    print(f"[filter {parity_label}] valid={n_valid}")
    print(f"[filter {parity_label}] filtered={n_bad} ({frac_bad:.4%})")
    print(f"[filter {parity_label}] Filtered: nh=0 or non-finite (out-of-window) samples.")

    if n_valid == 0:
        raise ValueError(f"No valid samples remain after filtering for {parity_label}. Check n_h window coverage.")
    ...
    # Reference: log(n_h * f)
    log_nhf = logf + np.log(nh)

    # Scatter: logq vs log(n_h * f)
    sc = ax_scatter.scatter(log_nhf, logq, c=nh, cmap="viridis", s=3, alpha=0.3, rasterized=True)
    cb = plt.colorbar(sc, ax=ax_scatter)
    cb.set_label(r"$n_h$", fontsize=fs_label)
    cb.ax.tick_params(labelsize=fs_tick)
    lmin = min(log_nhf.min(), logq.min())
    lmax = max(log_nhf.max(), logq.max())
    ax_scatter.plot([lmin, lmax], [lmin, lmax], "r--", lw=1, label="y = x")
    ax_scatter.set_xlabel(r"$\log(n_h \cdot |W(X)|)$", fontsize=fs_label)
...
def main():
    ...
    if cli.combined:
        ...
        # Filter
        mask_e = (nh_even > 0) & np.isfinite(logq_even) & np.isfinite(logf_even)
        mask_o = (nh_odd > 0) & np.isfinite(logq_odd) & np.isfinite(logf_odd)
        ...
        log_nhf_e = lf_e + np.log(nh_e)
        log_nhf_o = lf_o + np.log(nh_o)

        # Scatter: even (blues) and odd (reds)
        sc_e = ax_sc.scatter(log_nhf_e, lq_e, c=nh_e, cmap="winter", s=3, alpha=0.3, rasterized=True, label="even")
        sc_o = ax_sc.scatter(log_nhf_o, lq_o, c=nh_o, cmap="autumn", s=3, alpha=0.3, rasterized=True, label="odd")
        lmin = min(log_nhf_e.min(), log_nhf_o.min(), lq_e.min(), lq_o.min())
        lmax = max(log_nhf_e.max(), log_nhf_o.max(), lq_e.max(), lq_o.max())
        ax_sc.plot([lmin, lmax], [lmin, lmax], "k--", lw=1.5, label="y = x")
        ax_sc.set_xlabel(r"$\log(n_h \cdot |W(X)|)$", fontsize=fs_label)
        ax_sc.set_ylabel(r"$\log q(X)$", fontsize=fs_label)
        ax_sc.tick_params(labelsize=fs_tick)

        rho_e = np.corrcoef(log_nhf_e, lq_e)[0, 1]
        rho_o = np.corrcoef(log_nhf_o, lq_o)[0, 1]
        ...
        if cli.paper:
            ax_sc.set_title(r"$\log q$ vs $\log(n_h \cdot |W|)$", fontsize=fs_title)
        else:
            ax_sc.set_title(f"logq vs log(nh*|W|)  (even: {mask_e.sum()}, odd: {mask_o.sum()})", fontsize=fs_title)

        # Histogram: both parities
        logw_e = lq_e - log_nhf_e
        logw_o = lq_o - log_nhf_o
        ...
        ax_hist.legend(fontsize=fs_legend)
        if cli.paper:
            ax_hist.set_title(r"$\log(q / n_h |W|)$ distribution", fontsize=fs_title)
        else:
            ax_hist.set_title(r"$\log(q / n_h |W|)$ distribution", fontsize=fs_title)

        if cli.paper:
            fig.suptitle(r"Numerator model ($2\times2$, $\beta=8.0$)", fontsize=fs_title+2)
        else:

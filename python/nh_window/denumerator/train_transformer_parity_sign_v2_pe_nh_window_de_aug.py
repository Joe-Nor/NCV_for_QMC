#!/usr/bin/env python3
"""
Denominator Sign CV Training with standard CE loss, n_h Window,
and Cyclic Data Augmentation.

Adds automatic n_h truncation based on input data distribution.
At nmax, EOS is forced regardless of parity to preserve normalization.

Cyclic augmentation: each sample is randomly cyclic-shifted each epoch,
with parity_prefix recomputed via Fortran library.

Usage:
    python train_transformer_parity_sign_v2_pe_nh_window_de_aug.py --parity even --data_glob "/path/*.bin" --auto_nh_window 1
"""
    ...
    for fpath in file_paths:
        ...
        with open(fpath, 'rb') as f:
            ...
            while total < max_scan_samples:
                ...
                # Skip rest of record
                if fmt_version == 2:
                    skip_bytes = 4 + nh + 4 * nh  # K(4) + parity_prefix(nh) + opstring(4*nh)
                elif fmt_version == 3:
                    skip_bytes = 4 + 4 + 4 * nh   # K(4) + parity(4) + opstring(4*nh)
                else:  # V4
                    skip_bytes = 4 + 4 * nh        # parity(4) + opstring(4*nh)
                f.seek(skip_bytes, 1)
...
class RSSEStreamingDatasetV2(IterableDataset):
    ...
    def _scan_samples(self, f, file_size: int, header_size: int, fmt_version: int) -> Tuple[int, List[int]]:
        ...
        while pos < file_size:
            ...
            if fmt_version == 4:
                # V4: (nh:i4, parity:i4) = 8 bytes, no K
                nh_p_bytes = f.read(8)
                if len(nh_p_bytes) < 8:
                    break
                nh, _parity = struct.unpack("<2i", nh_p_bytes)
                skip_bytes = 4 * nh  # opstring only
            else:
                # V2/V3: (nh:i4, K:i4) = 8 bytes
                nh_k_bytes = f.read(8)
                if len(nh_k_bytes) < 8:
                    break
    ...
    def _parse_sample_from_file(self, f, fmt_version: int) -> Optional[Dict]:
        ...
        if fmt_version == 4:
            ...
            K = 0  # will be recomputed by Fortran lib
            ...
            parity_prefix = np.array([], dtype=np.int8)  # recomputed in collate
...
def collate_fn_parity_v2_aug(batch: List[Dict], bsites=None, nn_sites=None, nb_bonds=None, augment=True, cyclic_aug=True, spatial_perm=None, pointgroup_perm=None, compute_candidates=False):
    ...
    # Values: -1,0,+1 mapped to indices 0,1,2 via (delta+1)
    # BOS and post-EOS positions: index 1 (delta=0)
    deltaK_prefix_tensor = torch.ones((len(batch), input_seq_len), dtype=torch.long)  # default=1 (delta=0)
    for b, dkp in enumerate(deltaK_prefixes):
        nh = len(dkp)
        if nh > 0:
            dkp_tensor = torch.from_numpy(dkp.astype(np.int64)) + 1  # map -1,0,+1 → 0,1,2
            end = min(1 + nh, input_seq_len)
    ...
    if compute_candidates:
        ...
        for b, sample in enumerate(batch):
            ...
            if cand is None:
                continue
            # Usable prediction steps in inputs: positions 0..nh (that's nh+1 rows).
            # Cand matrix has shape (nh+1, nb).
            n_rows = min(cand.shape[0], input_seq_len)  # = min(nh+1, input_seq_len)
...
class AutoregressiveTransformer(nn.Module):
    ...
    def __init__(self, vocab_size, d_model=128, nhead=4, num_layers=4,
        ...
        self.deltaK_embedding = nn.Embedding(3, d_model)  # 3 values: -1,0,+1 mapped to 0,1,2
    ...
    def _init_weights(self):
        ...
        if self.dk_mlp_head:
            ...
            # Hidden linear: std=0.02 (matches rest of model)
            ...
            # starts at exactly 0 → model behaves as baseline at init → training
            # is stable; MLP then learns to add ΔK-aware corrections.
    ...
    def _compute_bond_correction(self, h, dk_candidates):
        ...
        h:              (B, T, d_model)
        dk_candidates:  (B, T, V) float32; bond slice at [:, :, OPERATOR_OFFSET:].
                        Expected values in {-1, 0, +1} in the bond slice.
        ...
        dk_idx = (bond_slice + 1).long().clamp(0, 2)  # (B, T, nb), ∈ {0,1,2}
        dk_emb = self.dk_head_embed(dk_idx)           # (B, T, nb, d_k)
        ...
        if self.dk_head_bond_emb > 0:
            ...
            b_emb = self.bond_id_embed(bond_ids)       # (B, T, nb, d_bond)
            cat_parts.append(b_emb)
        cat = torch.cat(cat_parts, dim=-1)            # (B, T, nb, d_model+d_k[+d_bond])
        bond_corr = self.bond_head(cat).squeeze(-1)    # (B, T, nb)
...
def compute_loss(model, batch, target_parity, pad_id, device, nmin, nmax,
    ...
    # Per-sample sequence NLL and token counts
...
def main():
    ...
    parser.add_argument('--dk_head_hidden', type=int, default=256,
                        help='Hidden dim of the MLP head (between concat input and scalar out).')
    parser.add_argument('--dk_head_centering', action='store_true',
                        help='Force bond_corr to be zero-mean across the 27 bond '
                             'candidates at each position. EOS slot stays 0, so '
                             'output_proj alone owns bond-vs-EOS balance — anchors '
                             'q+/q- to a shared calibration baseline.')
    ...
    if need_auto_scan:
        ...
        # Auto compute nmin/nmax using count-based cumulative distribution (denominator)
        auto_nmin, auto_nmax = choose_nh_window_from_hist(
            hist, args.left_tail_mass, args.right_tail_mass, weighted=False
        )
        print(f"  Auto n_h window (count-based): nmin={auto_nmin}, nmax={auto_nmax}")
    ...
    # Catches any indexing mismatch between Fortran output and the (B,T,V) tensor.
    if compute_candidates:
        ...
        for b_idx, s in enumerate(raw):
            ...
            x = s['x_dense']
            K = s['K']
            bonds = (x // 2).astype(np.int64) - 1  # 0-indexed bond per step, length nh
            tele = 0.0

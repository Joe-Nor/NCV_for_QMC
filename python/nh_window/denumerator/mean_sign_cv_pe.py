...
# -*- coding: utf-8 -*-
"""
Mean sign CV computation for RSSE V2 dataset (prefix parity embedding variant).
Uses separate files: one for c_star estimation, another for evaluation.

Same as mean_sign_cv.py, but loads models from train_transformer_parity_sign_v2_pe
which include the prefix parity embedding.
"""
    ...
    with open(meta["path"], "rb") as f:
        for idx in range(start, Ntot):
            ...
            if fmt_version == 4:
                ...
                K = None  # computed below from compute_parity_prefix
            else:
            ...
            if nh > 0:
                ...
                if K is None:
                    K = 0
                if need_candidates:
                    # Empty prefix → one candidate row, all bonds give ΔK=-1
...
def precompute_eval_tensors(samples, need_candidates: bool = False, nb_bonds: int = None):
    ...
    for MLP-head inference. nb_bonds must be provided in that case.
    """
    N = len(samples)
    ...
    for i, s in enumerate(samples):
        ...
        if need_candidates:
            ...
            if cand is not None:
                # cand: (nh+1, nb). Usable rows: 0..nh (including EOS-prediction step).
...
def build_arrays(samples, logq_even_arr, logq_odd_arr):
    ...
    return s_arr, logf_arr, logq_arr, logw_arr, logq_even_arr, logq_odd_arr, nh_arr, K_arr


def print_diagnostics(label, s_arr, logq_arr, logf_arr, logw_arr,
    ...
    if valid_any.sum() >= 2:
        ...
        if np.std(h_arr) > 0 and np.std(s_arr) > 0:
            rho_all = np.corrcoef(s_arr, h_arr)[0, 1]
            print(f"    Corr(s, h) [all]       = {rho_all:.6f}")
        else:
            print("    Corr(s, h) [all]       = nan")

        # valid_any rho: excludes samples with both q's = -inf
        h_valid = h_arr[valid_any]
        s_valid = s_arr[valid_any]
        if np.std(h_valid) > 0 and np.std(s_valid) > 0:
            rho_valid = np.corrcoef(s_valid, h_valid)[0, 1]
            print(f"    Corr(s, h) [valid_any] = {rho_valid:.6f}  (rho, key CV metric)")
        else:
            print("    Corr(s, h) [valid_any] = nan")
    else:
        print("    Corr(s, h) = nan (not enough valid samples)")
...
def main():
    ...
    ap.add_argument("--h_clip_factor", type=float, default=0.0,
                    help="For samples with |h| > h_clip_factor * median(|h_train|), zero out h "
                         "(skip CV correction, keep raw s). Threshold derived from train data. "
                         "Excluded from c* fit on train; sample count preserved on test, so E[s] "
                         "estimate stays asymptotically unbiased. 0.0 disables (default).")
    args = ap.parse_args()
    ...
    s_train, logf_train, logq_train, logw_train, _, _, nh_train, K_train = \
        build_arrays(train_samples, train_logq_even, train_logq_odd)

    print_diagnostics("Train data", s_train, logq_train, logf_train, logw_train,
                      train_logq_even, train_logq_odd, nh_train, K_train)

    # Control variate: h = (q_even - q_odd) / f
    ...
    # Optional |h| outlier filter. Threshold derived on train, applied to test.
    h_clip_thr = None
    fit_mask = np.isfinite(h_train)
    if args.h_clip_factor > 0:
        med_abs_h = float(np.median(np.abs(h_train[fit_mask])))
        h_clip_thr = args.h_clip_factor * med_abs_h
        h_ok = (np.abs(h_train) <= h_clip_thr)
        fit_mask = fit_mask & h_ok
        print(f"\n[h clip] factor={args.h_clip_factor}, median(|h_train|)={med_abs_h:.6g}, "
              f"threshold={h_clip_thr:.6g}")
        print(f"[h clip train] kept={int(h_ok.sum())}/{len(h_train)}, "
              f"dropped={int((~h_ok).sum())}")

    s_train_fit = s_train[fit_mask]
    h_train_fit = h_train[fit_mask]

    # c_star = Cov(s, h) / Var(h)
    obs0 = s_train_fit - s_train_fit.mean()
    h0 = h_train_fit - h_train_fit.mean()
    var_h = (h0 * h0).mean()
    c_star = float((obs0 * h0).mean() / max(var_h, 1e-30))

    if np.std(h_train_fit) > 0 and np.std(s_train_fit) > 0:
        corr_s_h_train = np.corrcoef(s_train_fit, h_train_fit)[0, 1]
    else:
        corr_s_h_train = float("nan")

    print(f"\n[c_star estimation]")
    print(f"  C = {C:.6f}")
    print(f"  c* = {c_star:.6f}")
    print(f"  Corr(s, h) on train samples = {corr_s_h_train:.6f}"
          + (" [after h clip]" if h_clip_thr is not None else ""))
    ...
    s_test, logf_test, logq_test, logw_test, _, _, nh_test, K_test = \
        build_arrays(test_samples, test_logq_even, test_logq_odd)

    print_diagnostics("Test data", s_test, logq_test, logf_test, logw_test,
    ...
    # All clips on test use zero-out semantics: keep sample, set h=0 to skip CV correction.
    # This preserves N and keeps E[s_cv] asymptotically unbiased (residual = c*·E[h_zeroed]).
    h_test = h_test.copy()
    nonfinite_h = ~np.isfinite(h_test)
    n_nonfinite = int(nonfinite_h.sum())
    if n_nonfinite > 0:
        h_test[nonfinite_h] = 0.0
        print(f"[finite filter test] {n_nonfinite}/{len(h_test)} non-finite h zeroed")

    if h_clip_thr is not None:
        h_above = (np.abs(h_test) > h_clip_thr)
        n_h_zeroed = int(h_above.sum())
        bias_h = float(c_star * h_test[h_above].sum() / max(len(h_test), 1))
        h_test[h_above] = 0.0
        print(f"[h clip test] threshold={h_clip_thr:.6g}, zeroed={n_h_zeroed}/{len(h_test)}")
        print(f"[h clip test] est. residual bias on E[s_cv] (= c*·sum(h_zeroed)/N) = {bias_h:.4g}")

    s_cv_test = s_test - c_star * h_test

    if np.std(h_test) > 0 and np.std(s_test) > 0:
        corr_s_h_test_all = np.corrcoef(s_test, h_test)[0, 1]
    else:
        corr_s_h_test_all = float("nan")

    mu_raw = float(s_test.mean())
    mu_cv = float(s_cv_test.mean())
    se_raw = float(s_test.std(ddof=1) / math.sqrt(n_test))
    se_cv = float(s_cv_test.std(ddof=1) / math.sqrt(n_test))
    var_raw = float(s_test.var(ddof=1))
    var_cv = float(s_cv_test.var(ddof=1))
    ...
    print(f"  C      = {C:.10g}")
    print(f"  Corr(s, h)_train [all]    = {corr_s_h_train:.6f}")
    print(f"  Corr(s, h)_test  [all]    = {corr_s_h_test_all:.6f}")
    print(f"  finite matched samples    = {n_test_finite}/{n_test}")

    print(f"\n  E[h]_test  = {Eh_test:.10g}")
    print(f"  SE[h]_test = {SEh_test:.6g}")

    print(f"\n  No CV:   E[s]      = {mu_raw:.10g}")
    print(f"           Var[s]    = {var_raw:.10g}")
    print(f"           SE[s]     = {se_raw:.6g}")

    print(f"\n  With CV: E[s_cv]   = {mu_cv:.10g}")
    print(f"           Var[s_cv] = {var_cv:.10g}")
    print(f"           SE[s_cv]  = {se_cv:.6g}")

    print(f"\n  Variance reduction Var[s]/Var[s_cv] = {var_reduction:.4f}")
    print(f"  SE reduction       SE[s]/SE[s_cv]   = {se_reduction:.4f}")
    print(f"{'='*60}")

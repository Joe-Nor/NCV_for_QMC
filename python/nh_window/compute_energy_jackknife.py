...
def compute_logq_for_samples(model, target_parity, samples, nmin, nmax,
    ...
    for start in range(0, len(samples), batch_size):
        ...
        # bsites=None: skip in-collate recompute; samples already have parity_prefix /
        # deltaK_prefix / (optionally) deltaK_candidates from read_samples.
...
def main():
    ...
    ap.add_argument("--force_cstar_num_zero", action="store_true", help="Force numerator c_star to 0")
    ap.add_argument("--force_cstar_denom_zero", action="store_true", help="Force denominator c_star to 0")
    args = ap.parse_args()
    ...
    if args.force_cstar_num_zero:
        c_star_num, C_num = 0.0, 0.0
        print(f"  c_star_num = {c_star_num:.6f} (forced to 0), C_num = {C_num:.6f}")
        del train_samples_num
    else:
    ...
    if args.force_cstar_denom_zero:
        c_star_denom, C_denom = 0.0, 0.0
        print(f"  c_star_denom = {c_star_denom:.6f} (forced to 0), C_denom = {C_denom:.6f}")
        del train_samples_denom
    else:
    ...
    # Jackknife analysis
    # Why jackknife for ratio observables:
    # - Ratio E = -A/B is nonlinear, so error propagation is non-trivial
    # - Jackknife naturally captures correlation between numerator and denominator
    # - Central value uses full-sample ratio (unbiased), SE from jackknife replicates

    def jackknife_ratio_from_bins(A_bins, B_bins, beta, nn, nb, eps=1e-14):
        """
        Compute jackknife SE for ratio observable E = -A/(beta*nn*B) + nb/(4*nn).

        Args:
            A_bins: per-bin means of numerator (n_bins,)
            B_bins: per-bin means of denominator (n_bins,)
            beta, nn, nb: system parameters
            eps: threshold for denominator protection

        Returns:
            E_full: central value from full-sample ratio
            E_se: jackknife standard error
        """
        n_bins = len(A_bins)
        A_full = A_bins.mean()
        B_full = B_bins.mean()

        # Central value from full sample
        if abs(B_full) < eps:
            return np.nan, np.nan, np.nan
        E_full = -A_full / (beta * nn * B_full) + nb / (4.0 * nn)

        # Jackknife replicates (leave-one-bin-out)
        E_loo = []
    ...
    print(f"  E/N (JK mean)      = {E_jk_raw:.10f}")
    print(f"  E/N (bias-correct) = {E_bc_raw:.10f} ± {E_se_raw:.10f}")
    print(f"")
    print(f"With CV:")
    print(f"  E/N (full)         = {E_full_cv:.10f} ± {E_se_cv:.10f}")
    print(f"  E/N (JK mean)      = {E_jk_cv:.10f}")
    print(f"  E/N (bias-correct) = {E_bc_cv:.10f} ± {E_se_cv:.10f}")
    print(f"")
    if exact is not None:
        print(f"Exact (ED): {exact:.10f}")
        print(f"Error (no CV, full):         {abs(E_full_raw - exact):.10f} ({abs(E_full_raw - exact)/E_se_raw:.2f} sigma)")
        print(f"Error (no CV, bias-correct): {abs(E_bc_raw - exact):.10f} ({abs(E_bc_raw - exact)/E_se_raw:.2f} sigma)")
        print(f"Error (CV, full):            {abs(E_full_cv - exact):.10f} ({abs(E_full_cv - exact)/E_se_cv:.2f} sigma)")
        print(f"Error (CV, bias-correct):    {abs(E_bc_cv - exact):.10f} ({abs(E_bc_cv - exact)/E_se_cv:.2f} sigma)")
    else:

...
!-----------------------!
 module twist_statistics
!-----------------------!
! Statistics for twist channel (delta_nl = 0) acceptance rate
 save
 integer(8) :: n_twist_attempts = 0      ! Total attempts with delta_nl=0
 integer(8) :: n_twist_accepts = 0       ! Accepted attempts with delta_nl=0
 integer(8) :: n_nontwist_attempts = 0   ! Total attempts with delta_nl!=0
 integer(8) :: n_nontwist_accepts = 0    ! Accepted attempts with delta_nl!=0
 integer(8) :: n_total_attempts = 0      ! Total update attempts

 contains

 subroutine reset_twist_stats()
   n_twist_attempts = 0
   n_twist_accepts = 0
   n_nontwist_attempts = 0
   n_nontwist_accepts = 0
   n_total_attempts = 0
 end subroutine reset_twist_stats

 subroutine print_twist_stats()
   real(8) :: twist_rate, nontwist_rate, overall_rate

   if (n_twist_attempts > 0) then
      twist_rate = dble(n_twist_accepts) / dble(n_twist_attempts)
   else
      twist_rate = 0.d0
   endif

   if (n_nontwist_attempts > 0) then
      nontwist_rate = dble(n_nontwist_accepts) / dble(n_nontwist_attempts)
   else
      nontwist_rate = 0.d0
   endif

   if (n_total_attempts > 0) then
      overall_rate = dble(n_twist_accepts + n_nontwist_accepts) / dble(n_total_attempts)
   else
      overall_rate = 0.d0
   endif

   write(*,'(A)') '=========================================='
   write(*,'(A)') 'Twist Channel Statistics'
   write(*,'(A)') '=========================================='
   write(*,'(A,I12)') 'Total attempts:        ', n_total_attempts
   write(*,'(A,I12,A,F8.5)') 'Twist (delta_nl=0):    ', n_twist_attempts, &
                              '  (', dble(n_twist_attempts)/dble(n_total_attempts), ')'
   write(*,'(A,I12,A,F8.5)') '  - Accepted:          ', n_twist_accepts, &
                              '  (rate: ', twist_rate, ')'
   write(*,'(A,I12,A,F8.5)') 'Non-twist (delta_nl!=0):', n_nontwist_attempts, &
                              '  (', dble(n_nontwist_attempts)/dble(n_total_attempts), ')'
   write(*,'(A,I12,A,F8.5)') '  - Accepted:          ', n_nontwist_accepts, &
                              '  (rate: ', nontwist_rate, ')'
   write(*,'(A,F8.5)') 'Overall acceptance:    ', overall_rate
   write(*,'(A)') '=========================================='
 end subroutine print_twist_stats

 end module twist_statistics
!--------------------------!
...
!================================!
 ...
 ! Reset twist statistics before measurement phase
 call reset_twist_stats()
 ...
 ! Print twist channel statistics
 call print_twist_stats()
...
!--------------------------------!
 ...
 do i = 0, mm-1
    ...
    if (op == 0) then
       ...
       ! Update statistics
       n_total_attempts = n_total_attempts + 1
       if (delta_nl == 0) then
          n_twist_attempts = n_twist_attempts + 1
       else
          n_nontwist_attempts = n_nontwist_attempts + 1
       endif
       ...
       if (ran() < prob_accept) then
          ...
          if (delta_nl == 0) then
             ...
             n_twist_accepts = n_twist_accepts + 1
          else
             n_nontwist_accepts = n_nontwist_accepts + 1
          endif
       else
          ! Reject: revert
